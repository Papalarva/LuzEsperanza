...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["registro_usuario"])) {
    ...
    } elseif (!preg_match("/^[a-zA-ZÁÉÍÓÚÜÑáéíóúüñ\s]+$/u", $primer_apellido)) {
...
if (empty($errores)) {
    ...
    try {
        ...
        $sql_paciente = "INSERT INTO pacientes (nombre, primerApellido, segundoApellido, fechaNacimiento, sexo, estadoNacimiento, CURP, telefono, correoElectronico)
                         VALUES ('$nombre', '$primer_apellido', '$segundo_apellido', '$fecha_nacimiento', '$sexo', '$estado_nacimiento', '$curp', '$telefono', '$correo')";
        ...
        <section class="login__contenedor">
            <form method="POST" action="" class="contacto__formulario registro__formulario" novalidate>
                ...
                <section class="registro__contenedor">
                    ...
                    <label for="primer_apellido" class="contacto__label">
                        <input type="text" name="primer_apellido" id="primer_apellido" class="contacto__input" required
                            value="<?= htmlspecialchars($primer_apellido ?? '') ?>">
                        <span class="contacto__placeholder">Primer Apellido</span>
                        <span class="contacto__error"><?= $errores['primer_apellido'] ?? '' ?></span>
                    </label>

                    <label for="segundo_apellido" class="contacto__label">
                        <input type="text" name="segundo_apellido" id="segundo_apellido" class="contacto__input"
                            value="<?= htmlspecialchars($segundo_apellido ?? '') ?>">
                        <span class="contacto__placeholder">Segundo Apellido</span>
                        <span class="contacto__error"><?= $errores['segundo_apellido'] ?? '' ?></span>
                    </label>
                    ...
                    <label for="fecha_nacimiento" class="contacto__label">
                        <input type="date" name="fecha_nacimiento" id="fecha_nacimiento" class="contacto__input"
                            required value="<?= htmlspecialchars($fecha_nacimiento ?? '') ?>" min="1925-01-01" max="<?= date('Y-m-d') ?>">
                    ...
                    <label for="telefono" class="contacto__label">
                        <input type="number" name="telefono" id="telefono" maxlength="10"  pattern="\d{10}" required class="contacto__input"
                ...
                <?php if ($mostrar_modal): ?>
                    <dialog id="registroModal" class="contacto__modal">
                        ...
                        <form method="GET" action="registrar.php">
                            <button type="button" class="contacto__boton" onclick="window.location.href='../index.php';">Iniciar sesión</button>
                        </form>
                    </dialog>
                <?php endif; ?>
            </form>
